...
def format_detection(skeyword, url):
    """
    ...
    secret_data.insert(0, "xGG_Public")
...
def check_existing_detections(url_list, search_query):
    ...
    try:
        ...
        configs.read_hashed_url(
            file_name=file_prefix + "public_hashed_url_custom_keywords.csv"
        )
    if url_list:
        for url in url_list:
...
def process_search_results(search_response_lines, search_query):
    ...
    hashed_urls_file = os.path.join(
        configs.output_dir, file_prefix + "public_hashed_url_custom_keywords.csv"
    )
            ...
            if secrets_detected:
                try:
                    ...
                    secrets_detected_df = pd.DataFrame(
                        secrets_detected,
                        columns=configs.xgg_configs["keywords"]["public_data_columns"],
                    )
                    detection_writes_per_query += secrets_detected_df.shape[0]
                    try:
                        secrets_detected_file = os.path.join(
                            configs.output_dir,
                            "xgg_public_custom_keywords_detected.csv",
                        )
...
def run_detection(public_keywords=[], org=[], repo=[], search_archived = True, search_forked = True):
    ...
    params: public_keywords - list - optional
    params: org - list - optional
    params: repo - list - optional
    returns: True or False
    """
    if public_keywords:
        if isinstance(public_keywords, list):
            configs.secondary_keywords = public_keywords
        else:
            logger.error(f"Please pass public_keywords in List like '['password',]'")
            sys.exit(1)
    else:
        # Get the secondary_keywords from secondary_keywords file
        configs.read_secondary_keywords(file_name="public_keywords.csv")
    logger.info(f"Total Public keywords : {len(configs.secondary_keywords)}")
    ...
    logger.info(f"Total No.of search queries: {len(search_query_list)}")
...
if __name__ == "__main__":
    ...
    # Check if the GitHub API token environment variable for "public" is set

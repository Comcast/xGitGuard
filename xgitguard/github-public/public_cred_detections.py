...
def check_existing_detections(url_list, search_query):
    ...
    try:
        ...
        configs.read_hashed_url(file_name="public_hashed_url_creds.csv")
    ...
    if url_list:
        ...
        if new_hashed_urls:
                    ...
                    if not secrets_ml_predicted.empty:
                        ...
                        try:
                            secrets_detected_file = os.path.join(
                                configs.config_dir, "xgg_public_creds_detected.csv"
                            )
                            write_to_csv_file(
            ...
            hashed_urls_file = os.path.join(
                configs.config_dir, "public_hashed_url_creds.csv"
            )
            try:

    ...
    if url_list:
        ...
        if new_hashed_urls:
                    ...
                    if not secrets_ml_predicted.empty:
                        ...
                        try:
                            secrets_detected_file = os.path.join(
                                configs.config_dir, "xgg_enterprise_keys_detected.csv"
                            )
                            write_to_csv_file(
            ...
            hashed_urls_file = os.path.join(
                configs.config_dir, "enterprise_hashed_url_keys.csv"
            )
            try:
...
if __name__ == "__main__":
    ...
    logger.info("xGitGuard Enterprise Keys and Token Detection Prcoess Started")

    run_detection(secondary_keywords, extensions, train_model)

    logger.info("xGitGuard Enterprise Keys and Token Detection Prcoess Completed")
